...
export class PostgresRepository implements IDBRepository {
  ...
  async getTableDDL(tableName: string) {
    return this.postgresClient.query(
      `SELECT triprofunc_generate_create_table_statement($1) as tabledef;`,
      [tableName]
    );
  }
  ...
  generateDropStatement(
    ...
    if (includeDrop) return `DROP ${type.toUpperCase()} IF EXISTS ${objectName};`;
    return '';
  }
  ...
  preloadUtilities() {
    //There isn't a direct way in postgres to return DDL so have to do this
    return this.postgresClient
      .query(`CREATE OR REPLACE FUNCTION triprofunc_generate_create_table_statement(p_table_name varchar)
                RETURNS text AS
                $BODY$
                DECLARE
                  v_table_ddl   text;
